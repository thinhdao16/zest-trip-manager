...
export default function ListProvider() {
  ...
  useEffect(() => {
    ...
    axiosInstance
      .get(`${BASE_URL}/user-wallet/withdraw`,)
      .then((response) => {
      ...
      {loading ? (
        ...
        <Container>
          <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
            <Typography variant="h4">List Provider Withdraw</Typography>
          </Stack>
                ...
                <Table sx={{ minWidth: 800 }}>
                  <UserTableHead
                    ...
                    headLabel={[
                      ...
                      { id: 'company', label: 'Bank Name' },
                      { id: 'role', label: 'Bank Account' },
                      { id: 'isVerified', label: 'Account Name' },
                  ...
                  <TableBody>
                    {dataFiltered
                      ...
                      ?.map((row) => (
                        <UserTableRow
                          key={row.id}
                          name={row?.UserWallet?.user?.Providers?.company_name}
                          role={row?.metadata?.bankAccountNumber}
                          status={row.status}
                          company={row?.metadata?.bankName}
                          avatarUrl={row.avatar_image_url}
                          isVerified={row?.metadata?.bankAccountName}
                          amount={row?.amount}
